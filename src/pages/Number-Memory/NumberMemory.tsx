...
import Game from '../../components/Games/Number-Memory/Game';

import './NumberMemory.css';
const rng = (min:number, max:number):string => {
  return Math.floor(Math.random() * (max-min + 1) + min).toString()
}

const NumberMemory = () => {
  const [ number, setNumber ] = useState(rng(1,10));
  ...
  useEffect(() => {
    setNumber(rng(max/10,max))
  },[max])

  useEffect(() => {
    if(classPrompt.length > 0){
      setTimeout(() => {setClassPrompt('')},300)
    }
  },[classPrompt])
  ...
  return (
    <div id='number-memory' className={`game-bg ${classPrompt}`}>
        <div className="container game-container">
            ...
            {(isPlaying && !gameOver && !showStats) && 
              <Game setGameOver={setGameOver} getAnswer={getAnswer} setPromptCorrect={setPromptCorrect} number={number}/>
            }
            ...
            {(gameOver || showStats) &&
              <div className='stats'>
                ...
                <div className="user-answer">
                  <h2 >Your Answer</h2>
                  <span className={gameOver ? 'wrong' : ''}>{answer}</span>
                </div>
                <h1>Level {level}</h1>
                <p></p>
                {showStats && 
                  <button className="game-btn" onClick={() => nextLevel()}>
                    NEXT
                  </button>
                }
                {gameOver && 
